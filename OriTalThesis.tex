...
The architecture we propose is a combination of several ANN layers types described below:
...
\textbf{Convolutional Neural Network Layer - CNN } CNN is a layer that utilize local correlation between adjacent cells of the input layer. Since, unlike FC layer, the output can be of multiple dimension, we refer to the output as \textit{Feature Map}. The feature maps are obtained by activating trainable multi-dimension kernels across the input layer. Fig.\ref{fig:LSTM_model} is an example of 1D CNN. The red, blue and green colors represent the trainable kernel weights across the input - layer $m-1$. Equation \ref{eq:1} describe the output of element $i$ in a 1D CNN layer feature map: 
...
\begin{table*}
	...
	\begin{tabular}{l|ccccccc}
		\toprule
		{subject} &   LDA &   \makecell{LSTM\\large} &  \makecell{LSTM-CNN\\large}&   CNN &  LSTM & \makecell{LSTM-CNN\\small}\\
		\midrule
...
\begin{table*}[t]
	...
	\begin{tabular*}{1.1\linewidth}{@{\extracolsep{\fill}}l|ccccc}
		\toprule
		{subject} & \makecell{LDA \\ all except one} &  \makecell{CNN\\all except one}&  \makecell{CNN\\all except one\\fine tune}& \makecell{LSTM-CNN\\SMALL\\all except one}&\makecell{LSTM-CNN\\SMALL\\all except one \\ fine tune} \\
		\midrule
